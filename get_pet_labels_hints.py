...
def get_pet_labels(image_dir):
    ...
    for idx in range(0, len(in_files), 1):
       ...
       if in_files[idx][0] != ".":
           ...
           # TODO 2a: Extract pet label from filename
           # Steps:
           # 1. Convert filename to lower case
           # 2. Split filename by underscores
           # 3. Filter only alphabetic words (ignore numbers and file extensions)
           # 4. Join filtered words with spaces
           filename = in_files[idx].lower()
           words = filename.split('_')
           pet_words = [word for word in words if word.isalpha()]
           pet_label = " ".join(pet_words).strip()

           # If filename doesn't already exist in dictionary add it and its pet label - otherwise print warning
           if in_files[idx] not in results_dic:
              results_dic[in_files[idx]] = [pet_label]
           else:
               print("** Warning: Duplicate files exist in directory:", in_files[idx])
 
    # TODO 2b: Return the results_dic dictionary
    return results_dic