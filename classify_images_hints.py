...
def classify_images(images_dir, results_dic, model):
    ...
    for filename in results_dic:
        # 3a. Run the classifier function
        full_path = images_dir + filename
        model_label = classifier(full_path, model)

        # 3b. Convert classifier label to lowercase and strip whitespace
        model_label = model_label.lower().strip()

        # 3c. Get the ground truth label (pet image label)
        truth = results_dic[filename][0]

        # 3c & 3d. Check if the pet label is found in classifier label
